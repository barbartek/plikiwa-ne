...
@app.route("/cleaning/delete", methodst=["POST"])
def cleaning_delete():
    if os.path.exists("data"):
        for root, dirs, files in os.walk("data", topdown=False):
            ...
            return render_template("cleaning.html"
                                   data_exist=False,
                                   massage="folder data już nie istnieje")
...
def daily_stats(date):#To jest funkcja obsługująca żądanie HTTP. Flask wywoła ją automatycznie, gdy użytkownik odwiedzi adres:
    ...
    with open(path, "r", encoding="utf-8") as file:#1. Otwiera plikpath — ścieżka do pliku, np. "./raw-data/2024-12-31.txt""r" — tryb read, czyli tylko do odczytuencoding="utf-8" — mówi Pythonowi, jak interpretować znaki (żeby polskie litery działały poprawnie) Po otwarciu pliku Python tworzy obiekt pliku i przypisuje go do zmiennej file. 
        ...
        text = file.read()#jest jedną z najprostszych, ale najważniejszych operacji przy pracy z plikami w Pythonie. W Twoim kodzie to właśnie tutaj zaczyna się cała analiza danych. Rozbijmy to na czynniki, żeby było naprawdę jasne. Co dokładnie robi file.read()? czyta cały plik naraz zwraca jego zawartość jako jeden duży string nie dzieli tekstu na linie — to zrobisz później (split("\n"))
        lines = text.split("\n") #robi jedną prostą, ale bardzo ważną rzecz: zamienia cały tekst pliku na listę linii. Co dokładnie się dzieje?Załóżmy, że text wygląda tak:Kod{'temp': 1.2}{'temp': 2.5}{'temp': 3.1}Po wykonaniu:pythonlines = text.split("\n")otrzymujesz listę:python["{'temp': 1.2}","{'temp': 2.5}","{'temp': 3.1}",""]
   
        readings = []#Tworzy pustą listę:
        for line in lines:#to kluczowy etap przetwarzania Twoich danych. Każda linia pliku jest tu czyszczona i zamieniana na słownik Pythona. Rozbijmy to na części, żebyś dokładnie wiedział, co się dzieje.
            clean_line = line.rstrip(",").replace("'",'"')#to kluczowy etap przetwarzania Twoich danych. Każda linia pliku jest tu czyszczona i zamieniana na słownik Pythona. Rozbijmy to na części, żebyś dokładnie wiedział, co się dzieje.
            readings.append(json.loads(clean_line))#to kluczowy etap przetwarzania Twoich danych. Każda linia pliku jest tu czyszczona i zamieniana na słownik Pythona. Rozbijmy to na części, żebyś dokładnie wiedział, co się dzieje.
